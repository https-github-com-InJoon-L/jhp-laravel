                    ...
                    <div class="mt-4">
                        <slot name="content">
                            ...
                            <div class="flex flex-col mb-4">
                                ...
                                <jet-input id="name" type="text" class="mt-1 block w-full" v-model="attend.desc_value" autocomplete="name" placeholder="지각, 병결 등"/>
                            </div>
                            <div class="flex flex-col mb-4">
                                <jet-label for="name" value="바퀴" />
                                <jet-input id="name" type="number" class="mt-1 block w-full" v-model="attend.run" autocomplete="run" placeholder="현재와 같거나 0이어야 합니다."/>
                            </div>
                        </slot>
                    </div>
                </div>
        ...
        methods: {
            ...
            change(){
                ...
                if(!this.attend.desc_value||this.attend.desc_value==''||
                    !this.attend.attend||this.attend.attend==''||
                    !((this.attend.run==0||this.attend.run=='0')||Number(this.attend.run)==Number(this.backupAttend.run))) {
                    if(!this.attend.desc_value||this.attend.desc_value==''){
                        errMsg.push('상태를 입력하지 않았습니다.');
                    }
                    if((!this.attend.run||this.attend.run=='')&&Number(this.attend.run)!=0){
                        errMsg.push('바퀴수를 입력하지 않았습니다.');
                    }
                    if(!((this.attend.run==0||this.attend.run=='0')||Number(this.attend.run)==Number(this.backupAttend.run))){
                ...
                errMsg.push('변경하였습니다.');
                axios.patch('/api/attends/'+this.user.id,this.attend)
                    .then(res=>{
                        console.log(res);
