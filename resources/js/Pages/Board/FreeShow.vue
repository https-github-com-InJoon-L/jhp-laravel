<template>
    <app-layout>
            ...
            <div class="px-2 py-4 bg-white dark:bg-gray-800 shadow rounded-lg max-w-lg  min-w-full">
                <div class="flex mb-4">
                    ...
                    <div class="ml-2 mt-0.5">
                        <span
                            class="block font-medium text-base leading-snug text-black dark:text-gray-100">{{ posts.title }}</span>
                                                    <span
                            class="block text-sm text-gray-500 dark:text-gray-400 font-light leading-snug">{{ posts.name }}</span>
                        <span
       ...
       <Dialog :show="commentDialog" @close="closeDialog">
        ...
        <template #footer>
            <button type="button"
                ...
                @click="deleteComment(deleteCommentID)">
                삭제
            </button>
            <button type="button"
        ...
        methods: {
            approve() {
                ...
                axios.patch("/api/run/" + this.posts.id, null, {
                    ...
                    .then(response => {
                        window.location.href = '/freeshow/' + this.postId + '?page=' + this.pageId;
                    })
            },
            ...
            del() {
                ...
                axios.delete("/api/posts/" + this.posts.id, {
                        params: value2
                    })
                    .then(response => {
                        ...
                        if (response.data.status != "false") {
                            window.location.href = '/free?page=' + this.pageId;
                        }
                    })
            },
                  closeDialog() {
            ...
            editpost() {
                window.location.href = '/freeedit/' + this.postId + '?page=' + this.pageId
            },
            submit() {
                let value = {
                    user_id: this.user_id,
                ...
                axios.post(`/api/post_free/${this.postId}/comment`, null, {
                        params: value
                    })
                    .then(response => {
                        alert("댓글 작성되었습니다");
            ...
            refresh() {
                axios.get("/api/posts/" + this.postId + '?page=' + this.commentPage)
                    .then(response => {
                        this.posts = response.data.post[0]
                        this.comments = response.data.comment.data
                        this.commentLinks = response.data.comment
            ...
            refreshPage(page) {
                ...
                axios.get("/api/posts/" + this.postId + '?page=' + page)
                    .then(response => {
            ...
            deleteComment(commentID) {
                axios.delete('/api/comment_free/' + commentID)
                    .then(response => {
                        alert("삭제 되었습니다")
                        this.refresh();
                        this.closeDialog();
            ...
            editComment(commentID, content) {
                axios.patch('/api/comment_free/' + commentID, {
                        content: content
                    })
                    .then(response => {
                        alert("수정 되었습니다")
