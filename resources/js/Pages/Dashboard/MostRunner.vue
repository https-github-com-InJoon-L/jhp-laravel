<template>
    ...
    <div class="card lg:card-side bordered compact">
        <div class="card-body">
            <h2 class="card-title font-semibolds">달리기 랭킹
                    ...
                    <tbody>
                        <tr v-for="(rank,i) in ranking" v-bind:key=i>
                            ...
                            <th class="text-center">
                                <progress class="progress progress-warning" v-bind:value="rank.totalRun"
...
<script>
    ...
    export default {
        ...
        mounted() {
            axios.get("/api/users/runners?page=" + this.currentPage)
                .then(response => {
                    this.ranking = response.data.runners.wdj.data
                    this.pageLinks = response.data.runners.wdj.links
                    this.data = response.data.runners.wdj;
                    this.from = response.data.runners.wdj.from
                    this.to = response.data.runners.wdj.to
                    this.total = response.data.runners.wdj.total
                    this.isLoading = 1;
                    this.currentPage = 1;
                    if (this.ranking[0]) {
                        this.max = this.ranking[0].totalRun
                    }
                })
        ...
        methods: {
            ...
            refreshByPage(page) {
                ...
                axios.get('/api/users/runners?page=' + page)
                    .then(response => {
                        this.ranking = response.data.runners.wdj.data
                        this.pageLinks = response.data.runners.wdj.links
                        this.data = response.data.runners.wdj;
