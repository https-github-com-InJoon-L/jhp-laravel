<template>
    ...
    <div class="card lg:card-side bordered compact">
        <div class="card-body">
            <h2 class="card-title font-semibolds">결석 랭킹
...
<script>
    ...
    export default {
        ...
        mounted() {
            axios.get("/api/users/absentees?page=" + this.currentPage)
                .then(response => {
                    this.ranking = response.data.absentees.wdj.data
                    this.pageLinks = response.data.absentees.wdj.links
                    this.data = response.data.absentees.wdj;
                    this.from = response.data.absentees.wdj.from
                    this.to = response.data.absentees.wdj.to
                    this.total = response.data.absentees.wdj.total
                    this.isLoading = 1;
                    this.currentPage = 1;
                    if (this.ranking[0]) {
        ...
        methods: {
            ...
            refreshByPage(page) {
                ...
                axios.get('/api/users/absentees?page=' + page)
                    .then(response => {
                        this.ranking = response.data.absentees.wdj.data
                        this.pageLinks = response.data.absentees.wdj.links
                        this.data = response.data.absentees.wdj;
