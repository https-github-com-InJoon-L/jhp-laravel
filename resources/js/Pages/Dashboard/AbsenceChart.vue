<template>
                    <div class="mx-auto">
                    <div class="card-body"> 
                    <div class="stat-title">최근 일주일 결석횟수</div>
                    <div class="stat-value">{{ sum }}</div>
                        <line-chart-red v-if="loaded" :height="200" :chart-data="count" :chart-labels="date" :options="options"  class="min-w-full"/>
                    </div>
                    </div>
</template>
...
<script>
    // import the styles 
import LineChartRed from './LineChartRed.vue'
    export default {
        data: () => ({
            loaded: false,
            ranking: [],
        ...
        mounted() {
            axios.get("/api/users/rank?attend=결석&teamId=2")
                .then(response => {
